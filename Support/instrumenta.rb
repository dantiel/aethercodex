...
INSTRUMENTA = [
  ...
  },
  {
    ...
    function: {
      ...
      parameters:  {
        ...
        properties: {
          message: { type: 'string' },
          level:   { type: 'string', enum: %w[info warn] }
        },
        required:   ['message']
      }
    }
  ...
  },
  {
    ...
    function: {
      name:        'file_overview',
      description: <<~DESCRIPTION,
        Fetch all information associated with a file (e.g., ai notes metadata and related file
        metadata, size, number of line, last modified).
      DESCRIPTION
      parameters:  {
        type:       'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name:        'aegis',
      description: <<~DESCRIPTION,
        The Aegis tool is for enabling an active context from Mnemosyne during conversations. When
        topic in current conversation changes you have to change or refine its state, ensuring
        relevance and precision in context note recall. Use it to refine the oracle’s focus. The
        Aegis tool will immediately return notes like `recall_notes` and keep them in context unlike
        `recall_notes` which only retrieves note for current interaction.
      DESCRIPTION
      parameters:  {
        type:       'object',
        properties: {
          tags:        { type: 'array', items: { type: 'string' } },
          summary:     { type:        'string',
                         description: 'Dynamic summary update without altering tags. Required for ' \
                                      'every invocation.' },
          temperature: { type:        'number',
                         description: 'Optional parameter to fine-tune the Aegis state ' \
                                      'responsiveness.' }
        },
        required:   []
      }
  ...
  },
  {
    ...
    function: {
      name:        'create_task',
      description: 'Generate a task for complex prompts with fields for plan, progress, and ' \
                   'max_steps.',
      parameters:  {
        type:       'object',
        properties: {
        ...
        required:   %w[plan max_steps title]
      }
    }
  },
  {
...
TASK_INSTRUMENTA = [
  {
    ...
    function: {
      name:        'reject_step',
      description: 'Marks a task step as failed.',
      parameters:  {
        type:       'string',
        properties: {
          reiterate_step: { type: 'string' },
          reason:         { type: 'string' }
        },
        required:   []
      }
  ...
  },
  {
    ...
    function: {
      name:        'complete_step',
      description: 'Completes the current step and proceeds to the next.',
      parameters:  {
        type:       'object',
        properties: {
