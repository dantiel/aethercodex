...
RSpec.describe TaskEngine do
  ...
  describe '#execute_task' do
    ...
    context 'with sub-tasks' do
      ...
      before do
        allow(Mnemosyne).to receive(:manage_tasks).with({ 'action' => 'list' }).and_return([task])
        allow(Mnemosyne).to receive(:manage_tasks).with({ 'action' => 'update', 'id' => 1, 'status' => 'active' })
        allow(Mnemosyne).to receive(:manage_tasks).with({ 'action' => 'update', 'id' => 1, 'progress' => 1 })
        allow(Mnemosyne).to receive(:manage_tasks).with({ 'action' => 'update', 'id' => 1, 'progress' => 2 })
        allow(Mnemosyne).to receive(:manage_tasks).with({ 'action' => 'list', 'parent_task_id' => 1 }).and_return([sub_task])
      end
